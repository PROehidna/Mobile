...
<GridLayout columns='*' rows='30, 50, auto'>
  ...
  <StackLayout v-if="hunger>0 || health>0 || mood>0" row='2' width='' orientation='vertical'>
    <Button class="button" text=">мали паук аркадий" @tap="edTierTwo()"/>
    <Button class="button" text=">паук аркадий" @tap="edTierThree()"/>
    <Button class="button" text=">больче паук аркадий" @tap="edTierFour()"/>
    <Button class="button" text=">оче больче паук аркадий" @tap="edTierFive()"/>
  </StackLayout>
