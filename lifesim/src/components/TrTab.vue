...
<GridLayout columns='*' rows='30, 50, auto'>
  ...
  <StackLayout v-if="hunger>0 || health>0 || mood>0" row='2' width='' orientation='vertical'>
    <Button v-if="!trTierTwoGtr" class="button" text=">мали паук аркадий (1000р)" @tap="trTierTwoAct()"/>
    <Button v-else isEnabled='false' class="button" text=">мали паук аркадий (куплена)"/>
    <Button v-if="!trTierThreeGtr" class="button" text=">паук аркадий (50000р)" @tap="trTierThreeAct()"/>
    <Button v-else isEnabled='false' class="button" text=">паук аркадий (куплена)"/>
    <Button v-if="!trTierFourGtr" class="button" text=">больче паук аркадий (100000р)" @tap="trTierFourAct()"/>
    <Button v-else isEnabled='false' class="button" text=">больче паук аркадий (куплена)"/>
    <Button v-if="!trTierFiveGtr" class="button" text=">оче больче паук аркадий (500000р)" @tap="trTierFiveAct()"/>
