...
<GridLayout columns='*' rows='30, 50, auto'>
  ...
  <StackLayout v-if="hunger>0 || health>0 || mood>0" row='2' width='' orientation='vertical'>
    <Button v-if="!esTierTwoGtr" class="button" text=">мали сычевальня (1000р)" @tap="esTierTwoAct()"/>
    <Button v-else isEnabled='false' class="button" text=">мали сычевальня (куплена)"/>
    <Button v-if="!esTierThreeGtr" class="button" text=">сычевальня (50000р)" @tap="esTierThreeAct()"/>
    <Button v-else isEnabled='false' class="button" text=">сычевальня (куплена)"/>
    <Button v-if="!esTierFourGtr" class="button" text=">больче сычевальня (100000р)" @tap="esTierFourAct()"/>
    <Button v-else isEnabled='false' class="button" text=">больче сычевальня (куплена)"/>
    <Button v-if="!esTierFiveGtr" class="button" text=">оче больче сычевальня (500000р)" @tap="esTierFiveAct()"/>
